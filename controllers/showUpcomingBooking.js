...
exports.showUpcomingBooking = async(req, res) => {
    try{
        ...
        bookings.forEach(booking => {
            if(booking.status == "Upcoming") return res.status(200).send({ bookings })
                else return res.status(404).send({ message: "Upcoming bookings are not found." })
        })
    }catch(error){
