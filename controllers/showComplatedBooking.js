...
exports.showComplatedBooking = async(req, res) => {
    try{
        ...
        bookings.forEach(booking => {
            if(booking.status == "Complate") return res.status(200).send({ bookings })
                else return res.status(404).send({ message: "Complate bookings are not found." })
        })
    }catch(error){
